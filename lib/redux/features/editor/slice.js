...
import { fetchAIdata, fetchDocumentById, saveDocumentById } from './thunks';
import { clExtractionPhases } from '@public/prompts/coverletter/index'; // CL Config
import { resumeextractionPhases} from '@public/prompts/resume/index'; // CV Config
...
const editorSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // --- Unified AI Data Fetch ---
      .addCase(fetchAIdata.fulfilled, (state, action) => {
        const { phaseKey, data } = action.payload;
        state.formDataMap = {
          ...state.formDataMap,
          [phaseKey]: data,
          designConfig: { ...state.formDataMap.designConfig }
        };
      });
  }
});
