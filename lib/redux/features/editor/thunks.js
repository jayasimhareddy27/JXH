...
export const fetchAIdataforDocument = createAsyncThunk(
  ...
  async ({ type, sectionIds, signal }, { dispatch, getState, rejectWithValue }) => {
    try {
      const { activeId } = getState().editor;
      const { allResumes } = getState().resumecrud;
      const { trackerListing,currentJob } = getState().jobsStore;
      const aiAgent = getState().aiAgent;
      let generateTask;
      
      let jobData = trackerListing.find(job => job.resumeId === activeId) || currentJob;
      let selectedResume = allResumes.find(r => r._id === currentJob?.resumeId) || allResumes.find(r => r._id === activeId);
      

      if (type === "resume") {
        generateTask = generateresumefromjobdata(  aiAgent,sectionIds, jobData, selectedResume,  dispatch, displayToast, signal);
      }else{
