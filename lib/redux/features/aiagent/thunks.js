  ...
  async (config, { rejectWithValue }) => {
    try {
      ...
      if (provider === 'ChromeAI') {
        const chromeModel = new ChromeAI({ temperature: 0.5, topK: 40 });
        response = await chromeModel.invoke(prompt);
        if (!response) throw new Error('Local AI via LangChain failed to respond.');
      ...
      if (provider !== 'ChromeAI' && !isExisting) {
        ...
        const syncResponse = await fetch('/api/userreferences', {
          method: 'PUT',
          headers: { 
