...
import { updateResumePhase, resetResumePhase, addResumePhaseItem, removeResumePhaseItem } from '@lib/redux/features/resumes/resumeeditor/slice';
...
const FieldList = memo(({ phase, formDataMap, renderField, isLoading }) => {
  ...
  const handleRemoveItem = (index) => {
    dispatch(removeResumePhaseItem({ phaseKey: phase.key, index }));
  };
    ...
    <>
      ...
      {phase.arrayFieldKey ? (
          ...
          <div key={item.id || index} className="border p-3 rounded-md bg-white shadow-sm relative mb-4 dark:bg-[color:var(--color-card-bg)]">
            {Object.entries(item).filter(([k]) => k !== "id").map(([key, val]) => {
              const updateItem = (updatedItem) => {
                ...
                dispatch(updateResumePhase({ phaseKey: phase.key, data: newArray }));
              };
        ...
        Object.entries(formData || {}).map(([key, val]) =>
          renderField([key, val], formData, (updatedData) => dispatch(updateResumePhase({ phaseKey: phase.key, data: updatedData })), null, isLoading)
        )
      )}
    </>
  );
...
const ActionButtons = memo(({ phase, formData, isLoading, handleFetchFromAI }) => {
  ...
  const handleAddItem = () => {
    ...
    dispatch(addResumePhaseItem({ phaseKey: phase.key, newItem }));
  };

  const handleReset = () => {
    dispatch(resetResumePhase(phase.key));
