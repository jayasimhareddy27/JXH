...
const ProfileForm = memo(({  phases,  expandedPhase,  isLoading,  formDataMap,  toggleAccordion,  handleFetchFromAI,  renderField,  handleSave,  scrollToPhase}) => {
  ...
  const btnClasses = "m-1 w-full px-4 py-2 bg-[color:var(--color-button-primary-bg)] " + "text-[color:var(--color-text-on-primary)] rounded-lg " + "hover:bg-[color:var(--color-button-primary-hover-bg)] " + "transition-colors duration-200 disabled:opacity-50";
      ...
      {phases.map((phase) => (
        <div key={phase.key} ref={(el) => (phaseRefs.current[phase.key] = el)}>
          <PhaseAccordion phase={phase} expandedPhase={expandedPhase} isLoading={isLoading} formDataMap={formDataMap} 
            toggleAccordion={toggleAccordion} handleFetchFromAI={handleFetchFromAI} renderField={renderField} />
        </div>
      ))}
      <button  disabled={isLoading}  onClick={handleSave}  type="button"  className={btnClasses}>
