...
import { loginUser, signupUser } from '@lib/redux/features/authslice';
...
export default function AuthPage() {
  ...
  // 1. Get user, loading, and error states from the Redux store
        ...
        <div className="text-center md:max-w-[80%]">
          <div className="flex justify-center md:gap-2 md:mb-6">
            {Companynameletters.map((letter, i) => (
              <span key={letter} className="text-5xl font-extrabold animate-[letterFade_1.5s_ease-in-out_infinite]" style={{ animationDelay: `${i * 0.3}s` }}>
                {letter}
              </span>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {!isLogin && (
              <div>
                ...
                <input id="name" type="text" name="name" value={form.name} onChange={handleChange} required placeholder="Your full name" className="form-input"/>
              </div>
            )}
            <div>
              <label htmlFor="email" className="block text-sm font-medium mb-1 text-[var(--color-form-label)]">Email</label>
              <input id="email" type="email" name="email" value={form.email} onChange={handleChange} required placeholder="you@example.com" className="form-input"/>
            </div>
            <div>
              <label htmlFor="password" className="block text-sm font-medium mb-1 text-[var(--color-form-label)]">Password</label>
              <input id="password" type="password" name="password" value={form.password} onChange={handleChange} required placeholder="Enter password" className="form-input"/>
            </div>
            {error && <p className="text-center text-sm text-red-600">{error}</p>}
            <button type="submit" disabled={isLoading} className={`form-button w-full py-3 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}>
          ...
          {isLogin ? (
            <>
              ...
              <p className="text-center text-sm text-[var(--color-paragraph)] mt-4">New user?{' '}
                <button className="text-[var(--color-form-button-bg)] hover:underline" onClick={() => setIsLogin(false)}>
                  Sign up
                </button>
              </p>
            </>
            ...
            <p className="text-center text-sm text-[var(--color-paragraph)] mt-4">Already registered?{' '}
              <button className="text-[var(--color-form-button-bg)] hover:underline" onClick={() => setIsLogin(true)}>
                Log in
              </button>
            </p>
          )}
