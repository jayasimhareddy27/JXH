...
export default function TemplateSelectionPage({templates}) {
  const dispatch = useDispatch();

  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');

  const document = useSelector(  (state) => state.editor.formDataMap);
  ...
  const handleTemplateSelect = (id) => {
    dispatch(updatePhase({ phaseKey: 'templateId', data: id }));
